...
function jfrefreshgrid(
	...
	for (let s = 0; s < range.length; s++) {
		...
		if (
			...
			let fp = $.trim(
				formula.functionParserExe(Store.flowdata[r1][c1].f)
			);
		...
		for (let m in cfg["merge"]) {
			...
			for (let r = mc.r; r <= mc.r + mc.rs - 1; r++) {
				for (let c = mc.c; c <= mc.c + mc.cs - 1; c++) {
					...
					if (r == mc.r && c == mc.c) {
						if (
							JSON.stringify(data[r][c].mc) != JSON.stringify(mc)
						) {
							data[r][c].mc = mc;
							mcData.push({ r: r, c: c });
						}
						...
						if (
							JSON.stringify(data[r][c].mc) !=
							JSON.stringify(tempMc)
						) {
							data[r][c].mc = tempMc;
							mcData.push({ r: r, c: c });
						}
	...
	if (calcChain != null && calcChain.length > 0) {
		...
		for (let i = 0; i < calcChain.length; i++) {
			...
			if (
				...
				let fp = $.trim(
					formula.functionParserExe(Store.flowdata[r][c].f)
				);
				let sparklines = new Function("return " + fp)();
