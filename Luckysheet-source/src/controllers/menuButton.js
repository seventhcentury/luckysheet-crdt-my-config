...
const menuButton = {
	...
	cancelPaintModel: function () {
		...
		if (
			Store.luckysheet_copy_save["dataSheetIndex"] ==
	...
	initialMenuButton: function () {
				...
				if ($menuButton.length == 0) {
					...
					$("#" + menuButtonId)
						...
						.spectrum({
							...
							noColorSelectedText:
								locale_toolbar.noColorSelectedText,
							localStorageKey:
								"spectrum.textcolor" + server.gridKey,
							palette: [
								[
									"#efeceb",
									"#f2f2f2",
							...
							change: function (color) {
								...
								$("#luckysheet-icon-text-color").attr(
									"color",
									color
								);
						...
						.click(function () {
							...
							if (Store.luckysheet_select_save.length > 1) {
								if (isEditMode()) {
									...
									tooltip.info(
										locale_alternatingColors.errorInfo,
										""
									);
								}
								return;
							...
							let isExists =
								alternateformat.rangeIsExists(range)[0];
							if (!isExists) {
				...
				$("#" + menuButtonId)
					...
					.spectrum({
						...
						change: function (color) {
							...
							// 下边框换成了一个DIV
							$(
								"#luckysheet-icon-cell-color .text-color-bar"
							).css("background-color", color);

							$("#luckysheet-icon-cell-color").attr(
								"color",
								color
							);
							let d = editor.deepCopyFlowData(Store.flowdata);
				...
				$menuButton
					...
					.click(function () {
						...
						if (!checkIsAllowEdit()) {
							tooltip.info(
								"",
								locale().pivotTable.errorNotAllowEdit
							);
							return;
						}
						$menuButton.hide();
		...
		$("#luckysheet-icon-font-size")
			...
			.click(function () {
				...
				if ($menuButton.length == 0) {
					...
					$menuButton
						...
						.click(function () {
							...
							$("#luckysheet-icon-font-size").attr(
								"itemvalue",
								itemvalue
							);
							_this.focus($menuButton, itemvalue);
				...
				let defualtvalue = $("#luckysheet-icon-font-size").attr(
					"itemvalue"
				);
		...
		$("#luckysheet-icon-border-menu").click(function () {
			...
			if ($menuButton.length == 0) {
				...
				$("#" + submenuid + " .luckysheet-cols-menuitem").click(
					function () {
						$("#" + submenuid).hide();
				...
				$menuButton
					...
					.click(function () {
						...
						let $t = $(this),
							itemvalue = $t.attr("itemvalue");
						if (
							itemvalue == "borderColor" ||
							itemvalue == "borderSize"
						) {
							return;
						}
						...
						let color = $("#" + subcolormenuid)
							.find(".luckysheet-color-selected")
							.val();
						let style = $("#luckysheetborderSizepreview").attr(
							"itemvalue"
						);
						...
						$("#luckysheet-icon-border-all").attr(
							"type",
							itemvalue
						);
				...
				$("#" + subcolormenuid)
					...
					.spectrum({
						...
						noColorSelectedText: locale_toolbar.noColorSelectedText,
						localStorageKey:
							"spectrum.bordercolor" + server.gridKey,
						palette: [
							[
								"#000",
								"#444",
						...
						if (Store.config["merge"] != null) {
							...
							for (
								...
								let r1 = Store.luckysheet_select_save[s].row[0],
									r2 = Store.luckysheet_select_save[s].row[1];
								let c1 =
										Store.luckysheet_select_save[s]
											.column[0],
									c2 =
										Store.luckysheet_select_save[s]
											.column[1];
						...
						if (itemvalue == "freezenRow") {
							...
							if (
								...
								if (isEditMode()) {
									...
									tooltip.info(
										locale_freezen.noSeletionError,
										""
									);
								}
						...
						if (itemvalue == "search" || itemvalue == "replace") {
							...
							if (
								...
								if (isEditMode()) {
									...
									tooltip.info(
										"",
										locale_findAndReplace.lessTwoRowTip
									);
								}
								return;
							}
							...
							if (
								...
								if (isEditMode()) {
									alert(
										locale_findAndReplace.lessTwoColumnTip
									);
								} else {
									tooltip.info(
										"",
					...
					.click(function () {
						...
						if (itemvalue == "if") {
							...
							if (
								...
								if (fp.indexOf("=if(") != -1) {
									...
									if (isEditMode()) {
										...
										tooltip.info(
											locale_formula.tipNotBelongToIf,
											""
										);
									}
									return;
							...
							if (cell != null && cell.f != null) {
								//单元格有计算
								let functionStr =
									luckysheetformula.getfunctionParam(cell.f);
					...
					.click(function () {
						...
						if (itemvalue == "icons") {
							if (Store.luckysheet_select_save.length == 0) {
								if (isEditMode()) {
									alert(
										conditionformat_text.pleaseSelectRange
									);
								} else {
									tooltip.info(
										conditionformat_text.pleaseSelectRange,
								...
								$.post(
									...
									function (d) {
										...
										for (let item in dataset) {
											if (
												item == Store.currentSheetIndex
											) {
												continue;
											}

											let otherfile =
												file[getSheetIndex(item)];

											otherfile.celldata =
												dataset[item.toString()];
											otherfile["data"] =
												sheetmanage.buildGridData(
													otherfile
												);
										}
										...
										conditionformat.fileClone = $.extend(
											true,
											[],
											file
										);
				...
				$(document)
					...
					.on(
						...
						function () {
							$menuButton.hide();
							$(
								"#luckysheet-icon-highlightCellRule-menuButton"
							).hide();
							luckysheetContainerFocus();
							...
							if (Store.luckysheet_select_save.length == 0) {
								...
								let textCellColorHtml =
									conditionformat.textCellColorHtml();

								let title, content;
								switch (itemvalue) {
									case "greaterThan":
										title =
											conditionformat_text.conditionformat_greaterThan;
									...
									case "lessThan":
										title =
											conditionformat_text.conditionformat_lessThan;
									...
									case "betweenness":
										title =
											conditionformat_text.conditionformat_betweenness;
									...
									case "equal":
										title =
											conditionformat_text.conditionformat_equal;
									...
									case "textContains":
										title =
											conditionformat_text.conditionformat_textContains;
									...
									case "occurrenceDate":
										title =
											conditionformat_text.conditionformat_occurrenceDate;
									...
									case "duplicateValue":
										title =
											conditionformat_text.conditionformat_duplicateValue;
								...
								conditionformat.conditionformatDialog(
									title,
									content
								);
							}
				...
				$(document)
					...
					.on(
						...
						function () {
							$menuButton.hide();
							$(
								"#luckysheet-icon-projectSelectRule-menuButton"
							).hide();
							luckysheetContainerFocus();
							...
							if (Store.luckysheet_select_save.length == 0) {
								...
								switch (itemvalue) {
									case "top10":
										title =
											conditionformat_text.conditionformat_top10;
									...
									case "top10%":
										title =
											conditionformat_text.conditionformat_top10_percent;
										content = `<div class="box" data-itemvalue="top10%">
                                                <div class="boxTitleOne">${conditionformat_text.conditionformat_top10_title}：</div>
                                                <div style="height: 30px;line-height: 30px;">
                                                    <div style="float: left;height: 30px;line-height: 30px;margin: 0 5px;">${conditionformat_text.top}</div>
									...
									case "last10":
										title =
											conditionformat_text.conditionformat_last10;
									...
									case "last10%":
										title =
											conditionformat_text.conditionformat_last10_percent;
										content = `<div class="box" data-itemvalue="last10%">
                                                <div class="boxTitleOne">${conditionformat_text.conditionformat_last10_title}：</div>
                                                <div style="height: 30px;line-height: 30px;">
                                                    <div style="float: left;height: 30px;line-height: 30px;margin: 0 5px;">${conditionformat_text.last}</div>
									...
									case "AboveAverage":
										title =
											conditionformat_text.conditionformat_AboveAverage;
									...
									case "SubAverage":
										title =
											conditionformat_text.conditionformat_SubAverage;
					...
					.on(
						...
						function () {
							...
							if (Store.luckysheet_select_save.length > 0) {
								...
								let format =
									conditionformat.dataBarList[itemvalue][
										"format"
									];

								conditionformat.updateItem(
									"dataBar",
									cellrange,
									format
								);
				...
				$(document)
					...
					.on(
						...
						function () {
							$menuButton.hide();
							$(
								"#luckysheet-icon-colorGradation-menuButton"
							).hide();
							luckysheetContainerFocus();
							...
							if (Store.luckysheet_select_save.length > 0) {
								...
								let format =
									conditionformat.colorGradationList[
										itemvalue
									]["format"];
			...
			.on(
				...
				function (e) {
					...
					if (e.type === "mouseover") {
						...
						_this.submenuhide[$attr.attr("id")] = setTimeout(
							function () {
								$attr.hide();
							},
							200
	...
	updateFormat: function (d, attr, foucsStatus) {
		...
		if (attr in inlineStyleAffectAttribute) {
			if (parseInt($("#luckysheet-input-box").css("top")) > 0) {
				...
				if (value.substr(0, 1) != "=") {
					let cell =
						d[Store.luckysheetCellUpdate[0]][
		...
		if (
			...
			if (!!margeset) {
				...
				if (!(rowseleted[1] < row_st || rowseleted[0] > row_ed)) {
					...
					//向右滑动或者居中时往左滑动的向下补齐
					if (
						columnseleted[1] >= col_st &&
						columnseleted[1] <= col_ed
					) {
						if (
							s.column_focus >= col_st &&
							s.column_focus <= col_ed
						) {
							width = col - left;
						} else {
							width = col - left;
	...
	getCellRealSize: function (d, cell_r, cell_c) {
		...
		if (!!celldata && celldata["mc"] != null) {
			...
			let config =
				getluckysheetfile()[getSheetIndex(Store.currentSheetIndex)][
					"config"
				];

			if (
				config["columnlen"] != null &&
				config["columnlen"][cell_c] != null
			) {
	...
	activeFormulaInput: function (
		...
		if (isnull) {
			...
			luckysheetformula.setCaretPosition(
				$span.get($span.length - 2),
				0,
				1
			);
	...
	autoSelectionFormula: function (formula) {
		...
		for (let s = 0; s < Store.luckysheet_select_save.length; s++) {
			...
			if (st_r == ed_r && st_c == ed_c) {
				...
				if (
					ed_r - 1 >= 0 &&
					_this.checkNoNullValue(d[ed_r - 1][st_c])
				) {
					execFormulaInput(d, st_r, ed_r, st_c, ed_c, formula);
				} else if (
					ed_c - 1 >= 0 &&
					_this.checkNoNullValue(d[st_r][ed_c - 1])
				) {
					execFormulaInput_c(d, st_r, ed_r, st_c, ed_c, formula);
				} else {
					execFormulaInput(d, st_r, ed_r, st_c, ed_c, formula);
	...
	fontInitial: function (fontList) {
		...
		if (fontList != null) {
			for (let a = 0; a < fontList.length; a++) {
				...
				if (
					document.fonts &&
					!document.fonts.check("12px " + fItem.fontName)
				) {
					if (fItem.url) {
