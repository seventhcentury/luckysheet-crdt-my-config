...
import WorkSheets from "./WorkerSheets";
import CellDatas from "./CellDatas";
import ConfigMerges from "./ConfigMerges";
import { logger } from "../../Meddlewear/Logger";
import ConfigBorders from "./ConfigBorders";
import ConfigHiddens from "./ConfigHiddens";

// 导出注册模型方法
function registerModel(sequelize: Sequelize) {
  WorkerBooks.register(sequelize);
  WorkSheets.register(sequelize);
  CellDatas.register(sequelize);
  ConfigMerges.register(sequelize);
  ConfigBorders.register(sequelize);
  ConfigHiddens.register(sequelize);
}

/**
 * 注意，这里的方法同步表，使用的是强制同步(force: true)，如果表不存在，则创建该表，如果表存在，则先删除，再创建
 *  这种方式同步的模型表，会导致表数据清空，请谨慎使用！！
 *  如果表内有外键关联，需要先删除外键表，后删除主键表，请注意使用顺序
 */
async function initModel() {
  // await WorkSheets.init();
  // await WorkerBooks.init();
}
